...
```{r}
districts %>% summarize(minGrad = min(grad), minGradDistrict = system_name[which.min(grad)])
```

6. Which district has the highest dropout rate?
Achievement School District
```{r}
districts %>% summarize(maxDropout = max(dropout), maxDropoutDistrict = system_name[which.max(dropout)])
```
...
filter(districts, region == "Mid Cumberland") %>% 
    summarize(maxValue = max(act_composite, na.rm = TRUE), maxName = system_name[which.max(act_composite)])
```
...
50 districts have a 80 percent or higher graduation rate. Approximately 10 districts have less than an 80 percent
graduation rate. Only one district has a less than 20 percent graduation rate.
```{r}
hist(districts$grad)
```
...
```{r}
plot(districts$alg_1, districts$alg_2)
```
...
```{r}
selected_data <- districts %>% select(alg_1, alg_2) %>% na.omit()
data <- data.frame(alg_1 = selected_data$alg_1, alg_2 = selected_data$alg_2)
data$alg_diff <- (data$alg_1 - data$alg_2)
```
